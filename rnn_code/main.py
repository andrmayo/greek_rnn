...
if __name__ == "__main__":
    ...
    if args.partition:
        ...
        logging.info(f"{len(json_list)} json files found: ")
        ...
        greek_char_data.write_to_json(train_json, train_data, random_index[: len(train_data)], mapping_greek_to_original)
    ...
    if args.eval:
        ...
        file_path = f"./data/" + test_json
        test_texts = utils.read_datafile(file_path) # returns of a list of utils.DataItem objects
        ...
        logger.info(
            f"File {test_json} read in with {len(test_texts)} texts"
        )
