...
import greek_rnn.letter_tokenizer as letter_tokenizer
from greek_rnn.greek_utils import DataItem, decoder_specs
...
class RNN(nn.Module):
	def __init__(
		...
		if caught_warnings:
			logger.info(
				"LSTM projection layer not using oneDNN optimization in __init__, "
				"which shouldn't matter when GPU is used for training/inference."
			)

		if self.share:
			if proj_size > 0 and proj_size != embed_size:
				raise ValueError(
					"if weight tying is enabled with share and projection layer, "
					"proj_size must equal embed_size"
				)
			elif hidden_size != embed_size:
				raise ValueError(
					"if weight tying is enabled with share and without "
					"projection layer, hidden_size must equal embed_size"
				)
			self.out = None
		else:
			self.out = nn.Linear(
	...
	def forward(
		...
		match self.decoder_type:
			...
			case _:
				raise ValueError(
					"invalid decoder requested; "
					f"decoder should be None or one of {list(decoder_specs.keys())}"
				)
	...
	def mask_and_label_characters(
		...
		# labels mark where actual masking is with a value > 0, and this value is
		# the index of the masked token
		data_item.labels = [-100] * text_length
		...
		if short_text:
			logger.info(
				"Encountered texts with less than 3 characters "
				f"(original position: {data_item.position_in_original}, "
				"which have not been masked"
			)
		data_item.mask = [True] * text_length

		# This counts only tokens actually masked,
		# not those swapped for random character or retained
		mask_count = 0
		random_sub = 0
		orig_token = 0
			...
			for i in range(text_length):
				...
				if r_mask_status < self.masking_proportion:
					if r_mask_type < 0.8:
						...
						# The integers that index to the embeddings
						# are in the closed interval [0, num_tokens - 1]
						replacement = random.randint(0, self.num_tokens - 1)
						random_sub += 1
					else:
						# retain original (these retained tokens are still True in mask
						# list of booleans)
		...
		# Total mask also includes tokens swapped for random character,
		# and tokens retained
		return (
			data_item,
			total_mask,
	...
	# This is the function to use to mask and label the test set incorporating
	# the actual lacunae as masks
	def actual_lacuna_mask_and_label(self, data_item: DataItem) -> DataItem:
		if not isinstance(data_item.text, str):
			raise TypeError(
				"DataItem passed to actual_lacuna_mask_and_label must "
				"have a string associated with it as text attribute"
			)
		text_buffer = []
		...
		for i in range(len(data_item.text)):
			...
			# Skip any characters not recognized by the tokenizer,
			# so that data_item.mask lines up with data_item.indexes
			if len(tokenized) == 0:
				continue
			if in_lacuna:
		...
		# self.labels with -100 for unmasked tokens and >= 0 with the embedding index
		# for masked tokens.
		return data_item
	...
	def mask_input(
		...
		"""
		Masking function: this is only used if masking
		is one-and-done at start of training.
		"""
...
class GRUDecoder(nn.Module):
	...
	def __init__(
		...
		# projection layer that projects concatenated
		# [biLSTM_context, prev_token_embedding] to the decoder hidden_size
		encoder_output_size = (
