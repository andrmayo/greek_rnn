...
def read_datafiles(
    ...
    json_blocks = []
    # maps position of text in list of all Greek texts to position of text
    # in list of all texts
    ...
    # in training_texts, all reconstructions are folded into text, missing characters
    # are left as '.', and gaps of unknown length are '!'
    # in lacuna_texts, all reconstructions are converted to the format
    # [# * number of missing characters], and gaps of unkown length are '!'
    ...
    for i, block in enumerate(json_blocks):
        training_texts.append(
            block["training_text"].translate({91: None}).translate({93: None})
        )
        training_texts[i] = re.sub("<gap/>", "!", training_texts[i])
        lacuna_texts.append(re.sub("<gap/>", "!", block["training_text"]))
        pattern = re.compile(r"\[.*?\]")
        lacuna_texts[i] = pattern.sub(
            lambda x: x.group().replace(" ", ""), lacuna_texts[i]
        )
    ...
    # partition the list:
    # random_index will keep track of where text stands in original json file
    random_index = [i for i in range(n_texts)]
    random.shuffle(random_index)

    # dev_data and test_data are coming from lacuna_texts because the masking
    # is actual lacunae, and character generation is evaluated against reconstructions
    train_data = [training_texts[i] for i in random_index[:train_length]]
    dev_data = [
        lacuna_texts[i]
        for i in random_index[train_length : train_length + dev_test_length]
    ]
    test_data = [
        lacuna_texts[i] for i in random_index[train_length + dev_test_length :]
    ]

    # create a list (for each text in json_blocks) of tuples
    # of format ("abcd[##]ghi[#]kl", ["ef", "j"])
    # In the few (7) cases where a lacuna in a Greek text has multiple
    # available reconstructions, the first one is used
    ...
    # random_index is in same order as train_data + dev_data + test_data,
    # and lists indexes to json_blocks
    # json_blocks contains texts in the order they stand in the json files,
    # and the json files in the order in which they stand in the file_list argument
    return (
        train_data,
        dev_data,
        test_data,
...
def write_to_json(
    ...
    with open(f"{cur_path}/data/{file_name}", "w") as jsonfile:
        for i, text in enumerate(text_list):
            json_block = {
                "text_index": text_index[i],
                "text": text,
                "position_in_original": mapping_greek_to_original[text_index[i]],
            }
