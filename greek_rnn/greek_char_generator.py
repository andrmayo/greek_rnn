...
def check_accuracy(target: list[int], orig_data_item: DataItem) -> tuple[int, int, int]:
    masked = 0
    correct = 0
    # We don't want to skip sentences with a restoration
    # of different length than the original text
    mismatch = 0
    ...
    # DataItem should have label[i] = -100 for unmasked tokens,
    # otherwise the label should give the embedding index for the masked token
    ...
    # masked is the # of masked tokens in input, correct is the number of predictions
    # that much the masked token, and mismatch returns 1 if model returned different
    # number of characters than there were characters in the lacunae
...
def train_model(
    ...
    # Now whether dev_data was passed or not, there's a train_data list object
    # and a dev_data list object (DataItems)
    ...
    for epoch in range(nEpochs):
        ...
        with typer.progressbar(
            ...
            for i in progress:
                ...
                if num_characters > 0:
                    logger.debug(
                        f"{epoch:4} {i:6} {num_tokens:5} {num_characters:6} "
                        f"loss {loss / num_tokens:7.3f} "
                        f"{loss / num_characters:7.3f} -- "
                        f"tot tr loss: {train_loss / train_tokens:8.4f} "
                        f"{train_loss / train_chars:8.4f}"
                    )
        ...
        (
            ...
            masking_strategy,
            update=False,
            dynamic_remask=False,  # Dev set is already masked, since lacunas masked
        )

        if epoch == 0:
            logger.info(
                f"train={len(train_list):,} {train_tokens:,} {train_chars:,} "
                f"{train_chars / train_tokens:0.1f} "
                f"dev={len(dev_list):,} {dev_tokens:,} {dev_chars:,} "
                f"{dev_chars / dev_tokens:0.1f} "
        ...
        logger.info(
            f"{epoch} tr loss {msg_trn} -- dev loss {msg_dev} -- "
            f"incremental_batch_size: {incremental_batch_size:4} "
            f"time elapsed: {time.time() - start:6.1f}"
        )
    ...
    # dev_data is being passed in here in the same format
    # as training data is passed to RNN model
...
def accuracy_evaluation(model: RNN, data: List[DataItem], data_indexes):
    ...
    if masked_total > 0:
        logger.info(
            f"masked total: {masked_total}, correct predictions: {correct}, "
            f"simple accuracy: {round(correct / masked_total, 3)}, "
            f"mismatch: {mismatch_total}"
        )
    else:
        logger.info(
            f"masked total: {masked_total}, correct predictions: {correct}, "
...
def baseline_accuracy(model: RNN, data: list[DataItem], data_indexes: list[int]):
    ...
    try:
        ...
        # return a dictionary of format {bigram: {char1: count, char2: count, ...}}
        # that tells how often each character completes the trigram
    ...
    for i in data_indexes:
        ...
        if data_item.labels is None:
            raise ValueError(
                "data passed to baseline_accuracy contains DataItem "
                "with unitialized labels"
            )
        ...
        for j in range(len(data_item.labels)):
            if data_item.labels[j] > 0:
                ...
                if data_item.indexes is None:
                    raise ValueError(
                        "data passed to baseline_accuracy contains "
                        "DataItem with unitialized indexes"
                    )
    ...
    logger.info(
        f"Most Common Char Baseline; dev masked total: {masked_total}, "
        "correct predictions: {correct_most_common_char}, "
        "baseline accuracy: {round(correct_most_common_char / masked_total, 3)}"
    )
    logger.info(
        f"Random Baseline; dev masked total: {masked_total}, "
        "correct predictions: {correct_random}, "
        "baseline accuracy: {round(correct_random / masked_total, 3)}"
    )
    logger.info(
        f"Trigram Baseline; dev masked total: {masked_total}, "
        "correct predictions: {correct_trigram}, "
        "baseline accuracy: {round(correct_trigram / masked_total, 3)}"
    )
...
# if save_to_file=True and output_file=None
def predict_top_k(
    model: RNN,
    data_item: DataItem,
    k: int = 10,
