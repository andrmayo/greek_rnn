...
def setup_logging():
    ...
    file_handler = logging.FileHandler(f"{Path(__file__).parent}/log/greek_rnn.log")
    file_handler.setLevel(logging.INFO)
    debug_file_handler = logging.FileHandler(
        f"{Path(__file__).parent}/log/greek_rnn_debug.log"
    )
...
default_model_name = "greek_rnn_model"
...
# in this case we might want learning rate to be configurable here
# for lower rate in phase 2, perhaps to 0.001
@app.command()
def train(
    masking_strategy: Annotated[
        ...
        typer.Argument(
            help=(
                "pass 'random' for randomly picking single characters to mask, "
                "and 'smart' for masking randomly sized sequences"
            ),
        ),
    ],
    ...
    force_partition: Annotated[
        ...
        typer.Option(
            ...
            "-f",
            help=(
                "By default data is only partitioned if partitions don't exist "
                "or if a change in the source data is detected "
                "(and there's data available to be partitioned)"
            ),
        ),
    ] = False,
    ...
    seq_decoder_type: Annotated[
        ...
        typer.Option(
            ...
            "--sequence-decoder",
            help=(
                "Pass in name of decoder, e.g. gru; "
                "if omitted, decoding is done with linear layer."
            ),
        ),
    ] = None,
    save_dir: Annotated[
    ...
    model_name: Annotated[
        str | None, typer.Option("--model-name", "-mn", help="Save model as <name>.pth")
    ] = None,
):
    ...
    # if masking_strategy is "dynamic",
    # utils.mask_input returns the data as passed to it, and mask = True
    # if masking_strategy is "once",
    # utils.mask_input adds masking to data by calling
    # model.mask_and_label_characters(),
    ...
    # if mask = True,
    # train_model() will call model.mask_and_label_characters() to remask.

    logger.info("Now training LSTM")
    logger.info(
        f"Masking strategy: {masking_strategy}\nTokens are masked: "
        f"{'dynamically' if dynamic_remask else 'once'}"
    )
    if not dynamic_remask:
        train_data = model.mask_input(train_data, masking_strategy)
    # mask ultimately gets passed to greek_char_generator.train_batch(),
    # to tell it whether to remask, which it should do if strategy is "dynamic"

    # actual_lacuna_mask_and_label replaces all characters within [] with '_', strips
    # '[' and ']', and adds embedding indices to self.labels for
    ...
    model = train_model(
        ...
        # true if masking_strategy is dynamic, false if masking_strategy is once
...
def predict(
    sentence: Annotated[
        ...
        typer.Argument(
            help=(
                "sentence with lacuna to fill marked as [..] "
    ...
    """
    Returns top reconstruction of Greek sentence with lacunae in format [..]
    with one . per missing character.
    """
    logger = logging.getLogger()
...
def predict_k(
    ...
    """
    Returns top k reconstruction of Greek sentence with lacunae
    in format [..] with one . per missing character.
    """
    if k == -1:
        k = typer.prompt(
            "Please enter an integer k for the number of predictions "
            "to return (from most to least probable)",
            type=int,
        )
...
def rank(
    sentence: Annotated[
        ...
        typer.Argument(
            help=(
                "sentence with one lacuna to fill marked as [..] "
                "with one . per missing character"
            ),
        ),
    ],
    options: Annotated[
        ...
        typer.Argument(
            help=(
                "space-separated options for filling lacuna "
                "(no spaces in reconstruction itself)"
            ),
        ),
    ] = None,
):
    ...
    Pass Greek sentence with lacunae marked as [..]
    followed by reconstructions to rank (without spaces or diacritics).
    """
    if options is None:
        opt_str = typer.prompt(
            "Please enter options of same length as lacuna, "
            "separated by spaces with quotation marks if needed:",
            type=str,
        )
...
def run_eval(model: greek_model.RNN, logger: logging.Logger):
    ...
    # these indexes tell where to look in reconstructed_lacunae
    # to get gold standard scholarly reconstructions
    test_indexes = [d.text_number for d in test_data if isinstance(d.text_number, int)]
    ...
    # now test_texts gives us our texts with lacunae as masks in the format [##]
    # for all the texts in the test partition (in random order).
    # reconstructed_lacunae is a list of lists giving all lacunae reconstructions
    # for all texts in json_blocks
    # note that reconstructions are in the order of the original json_blocks,
    # while test_texts is in the random order from the dataset partition.
    ...
    logger.info(
        f"{num_reconstructions} reconstructed lacunae read in accross "
        f"{num_texts_with_reconstructions} texts"
    )
...
def check_auto_partition(logger: logging.Logger) -> bool:
    if not json_list:
        logger.info(
            "Can't repartition, because there are "
            f"no source data files in {file_dir_path}"
        )
        return False
    for file in json_list:
        file = str(file)
...
def bind_data(
    ...
    # -> lacunae of known length are present as "[_]" with '_' * number of lacuna chars.
    ...
    # reconstructions: list of lists, for [a, b], a is a list of reconstructions
    # for the lacunae in the first text in the original json file.
    # To find the reconstructions for a text in one of the data files,
    # take its index i in [train_data + dev_data + test_data],
    # then take random_index[i], then take reconstructions[random_index[i]].
    ...
    if random_index is None or position_in_original is None or force_rebind:
        ...
        # Make it possible to match each text in the partition to a block in original
        ...
        # get the file in same order as original_json
        # with scholarly reconstructions of lacunae
        reconstructions = []
    ...
    # Now, convert each of train_data, dev_data, and test_data
    # into lists of objects of class greek_utils.DataItem.
